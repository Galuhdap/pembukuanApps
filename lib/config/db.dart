...
class DatabaseService {
  ...
  Future _onCreate(Database db, int version) async {
    ...
    await db.execute(
        '''CREATE TABLE $table6 (id INTEGER PRIMARY KEY,kode_invoice STRING NULL ,nama STRING NULL, nama_pembeli STRING NULL, produk TEXT NULL, jumlah_produk INTEGER NULL, total_produk INTEGER NULL, subtotal INTEGER NULL, ongkos_kirim INTEGER NULL, biaya_lain INTEGER NULL, potongan_harga INTEGER NULL,pembayaran_awal INTEGER NULL, total INT NULL,pembayaran TEXT NULL, jatuh_tempo TEXT NULL, createdAt TEXT, updatedAt TEXT)''');
    await db.execute(
  ...
  Future<List<PenjualanModel>> allPen() async {
    final data = await _database!.rawQuery("SELECT * FROM penjualan WHERE pembayaran IN ('Tunai', 'Transfer', 'Lunas') GROUP BY kode_invoice");
    print(data);
  ...
  Future<List<PenjualanModel>> hutangPen() async {
    final data = await _database!.rawQuery("SELECT * FROM penjualan WHERE pembayaran = 'Hutang' GROUP BY kode_invoice");
    List<PenjualanModel> result =
        data.map((e) => PenjualanModel.fromJson(e)).toList();
    return result;
  }
